...
//4
export function addMarksForProject(pairsAndThemes){
    const marksForProject = [];
    for(let i = 0; i < pairsAndThemes.length; i++){
        marksForProject.push(`<br>${pairsAndThemes[i]}: ${getRandomMark()}`);
    }
...
export const pairsAndThemes = connectPairsAndThemes(pairs, themes);